...
export class MockMemoryAdmissibilityTransport {
    blockedMemoryIds;
    merkleRoot;
    snapshotVersion;
    constructor(options) {
...
export class GrpcMemoryAdmissibilityTransport {
    generatedClient;
    constructor(generatedClient) {
        this.generatedClient = generatedClient;
    }
    async evaluateMemoryUse(_request) {
...
export function tombstoneForDecision(decision) {
    if (decision.decisionCode === DECISION_HARD_REFUSE)
        return HARD_REFUSE_TOMBSTONE;
    if (decision.decisionCode === DECISION_QUARANTINE)
        return QUARANTINE_TOMBSTONE;
    if (decision.decisionCode === DECISION_DELETE_REQUESTED)
        return DELETE_REQUESTED_TOMBSTONE;
    if (decision.decisionCode === DECISION_REFRESH)
        return REFRESH_TOMBSTONE;
    if (decision.decisionCode === DECISION_ESCALATE)
        return ESCALATION_TOMBSTONE;
    if (decision.decisionCode === DECISION_REFUSE)
        return REFUSE_TOMBSTONE;
    if (decision.decisionCode === DECISION_IGNORE)
